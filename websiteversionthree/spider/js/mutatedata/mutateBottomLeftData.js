    ...
    function addBottomLeftCenterBranch(){
        ...
        function checkDatasetOne (){   
            for (let i = 0; i < size; i++) {
                ...
                &&  ((dataset[i].value[1]+dataset[i].value[3] ) == (bottomLeftCenterBranch[1]+bottomLeftCenterBranch[3])) )
    ...
    function addBottomLeftTopBranch(){
        let bottomLeftTopBranch = [numCXVariable-300, 0,0, numCYVariable,0];
        var size = Object.size(dataset);
            function checkDatasetTwo (){   
                for (let i = 0; i < size; i++) {
                    if (      
                        ((dataset[i].value[0]+dataset[i].value[2] )== (bottomLeftTopBranch[0]+bottomLeftTopBranch[2]))
                    &&  ((dataset[i].value[1]+dataset[i].value[3] ) == (bottomLeftTopBranch[1]+bottomLeftTopBranch[3])) )
                        return true;
                         } 
                      return false; 
                     }
    ...
    function addBottomLeftRightBranch(){
        let bottomLeftRightBranch = [numCXVariable, 0,0,numCYVariable+300,0];
        var size = Object.size(dataset);
    function checkDatasetThree (){       
        for (let i = 0; i < size; i++) {
            if (      
                ((dataset[i].value[0]+dataset[i].value[2] )== (bottomLeftRightBranch[0]+bottomLeftRightBranch[2]))
            &&  ((dataset[i].value[1]+dataset[i].value[3] ) == (bottomLeftRightBranch[1]+bottomLeftRightBranch[3])) )
                return true;
                 } 
              return false; 
             }
            ...
            dataset.push({
               ...
               return dataset;
                        ...
                        for (var i = 0; i < size; i++) { 
                            ...
                            (bottomLeftRightBranch[1]+bottomLeftRightBranch[3])){
                            let position = dataset[i].value[3]+300;
                            dataset[i].value[2] = position;
                           }}
