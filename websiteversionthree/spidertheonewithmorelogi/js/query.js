...
var bigNum = querysearch.length + 20;
var smlNum = querysearch.length + 10;

  function loadMoreContent(){
        
    if (querysearch.length<bigNum){
        if (querysearch.length <smlNum ){
            if (xhr.readyState==4){
                ...
                var searchResults1 = JSON.parse(res1);               
                    for (var i = 0; i < 10; i++) {
                        var item1 = searchResults1.items[i];          
                            for (b=0; b < querysearch.length; b++)
                                if (item1.pagemap.cse_thumbnail[0].src == querysearch[b][0]){
                                console.log("got a match with search results");
                                }else{
            ...
            if (this.status ==200){
                ...
                var searchResults2 = JSON.parse(res2);
                  ...
                  for (var i = 0; i < 10; i++) {
                    ...
                    for (b=0; b < querysearch.length; b++)
                                if (item2.pagemap.cse_thumbnail[0].src == querysearch[b][0]){
                                console.log("got a match with search results");
                                }else{
